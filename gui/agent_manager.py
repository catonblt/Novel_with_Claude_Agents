    ...
    def start_conversation(self, agent_num: str, story_context: Dict) -> List[Dict]:
        ...
        if instructions:
            ...
            if project_files:
                ...
                context_parts.append("**IMPORTANT - Creating and Updating Files:**\n\n")
                context_parts.append("**Auto-Save Requirements:**\n")
                context_parts.append("- Chapters will auto-save ONLY when your response:\n")
                context_parts.append("  * Starts with a heading like `# Chapter 1` or `# Chapter One`\n")
                context_parts.append("  * Contains actual narrative prose (not discussion about writing)\n")
                context_parts.append("  * Is at least 1500 characters of story content\n")
                context_parts.append("  * Has multiple paragraphs with dialogue or narrative verbs\n")
                context_parts.append("- If updating an existing chapter, the user will be asked to confirm the overwrite\n")
                context_parts.append("- Discussion about chapters (e.g., 'Let me revise Chapter 3...') will NOT auto-save\n\n")
                context_parts.append("**Best Practices:**\n")
                context_parts.append("- When writing actual chapter content, start with the chapter heading and write the full narrative\n")
                context_parts.append("- When discussing changes, avoid starting with chapter headings - just discuss the ideas\n")
                context_parts.append("- For revisions, write out the complete revised chapter content, not just snippets\n")
                context_parts.append("- All auto-saves create version backups - previous versions are never lost\n\n")
